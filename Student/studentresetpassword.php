<?php
	session_start();
	if(!isset($_SESSION['student-token']) || !isset($_SESSION['student-email']) ){
		header('Location: studentlogin.php?error=notokenavailable');
		exit();
	}
?>
<!DOCTYPE html>
		...
		<div class="card border-success" style="width: 25rem;">
			...
			<div class="card-body">
			    <form  method="POST">
				  	...
				  	<button type="submit" class="btn btn-outline-danger "  name="student-back">Back</button>
...
<?php
	...
	if(isset($_POST['student-back'])){
		unset($_SESSION['student-token']);
		unset($_SESSION['student-email']);
		...
		if(empty($token) || empty($newpassword) || empty($repassword)){
			header('Location: studentresetpassword.php?error=emptyfields');
			exit();
		}
		else if($token != $_SESSION['student-token']){
			header('Location: studentlogin.php?error=invalidtoken');
			exit();
		}
		else if($newpassword != $repassword){
			header('Location: studentresetpassword.php?error=confirmpassword');
			exit();
		}
		else{
			$sql = "SELECT password FROM student WHERE emailid=?;";
			...
			if(mysqli_stmt_fetch($statement)){
				...
				if($pwdcheck == true){
					header('Location: studentresetpassword.php?error=sameoldpassword');
					exit();
				}
				else{
					$sql = "UPDATE student SET password=? WHERE emailid=?;";
					$statement = mysqli_stmt_init($connection);
					mysqli_stmt_prepare($statement,$sql);
					...
					if(mysqli_stmt_execute($statement)){
						unset($_SESSION['student-token']);
						unset($_SESSION['student-email']);
						header('Location: studentlogin.php?reset=successful');
						exit();
						
		
					}
					else{
						header('Location: studentresetpassword.php?reset=fail2');
						exit();
					}
				...
				header('Location: studentlogin.php?reset=fail1');
				exit();
			}
