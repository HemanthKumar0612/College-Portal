		...
		<div class="card border-success" style="width: 25rem;">
			...
			<div class="card-body">
			    <form  method="POST">
				  	...
				  	<a href="studentindex.php" class="btn btn-outline-success" role="button">Back</a>
				  	<button type="submit" class="btn btn-outline-danger "  name="change-password">Send Link</button>
				</form>
			</div>
		</div>
	</div>
...
<?php
	function resetpasswordmail($email,$token){
		...
		$msg=
			"<div>
				...
				<a href='http://localhost/Student%20database%20Management%20system/Student/studentresetpassword.php'>Reset Link</a>
				<p>One Time Token :</p>
	...
	if(isset($_POST['change-password'])){
		...
		if(mysqli_stmt_fetch($statement)){
			...
			if($pwdcheck == false){
				header("Location: studentchangepassword.php?error=wrongpwd");
				exit();
			}
			$token = openssl_random_pseudo_bytes(10);
			$token = bin2hex($token);
			session_start();
			$_SESSION['student-token']=$token;
			...
			header('Location: studentchangepassword.php?error=usernotfound');
			exit();
		}
